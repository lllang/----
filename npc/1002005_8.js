...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			text += "#L1##v4001028##t4001028# 兑换 #r血量#k    比例#r 1:50#k#l\r\n";
			text += "#L0##v4001028##t4001028# 兑换 #b蓝量#k    比例#r 1:50#k#l\r\n";
			...
			if (selection == 0) {
				cm.sendGetNumber("请输入你要兑换的数量:\r\n\r\n#v4001028##t4001028# 兑换 抵用  比例 1:50",0,1,214755885);
				selec = 0;
			} else if (selection == 1) {
				cm.sendGetNumber("请输入你要兑换的数量:\r\n\r\n#v4001028##t4001028# 兑换 点卷  比例 1:50",0,1,214755885);
				selec = 1;
			} else if (selection == 2) {
			...
			if(selec == 0){
				if(cm.haveItem(4001028,selection*1)){
					cm.gainItem(4001028,-selection*1);
					cm.getPlayer().getStat().setMaxMp((cm.getPlayer().getStat().getMaxMp() + selection*50));
					cm.刷新状态();
					cm.dispose();
                }else{
					cm.sendOk("材料不足");
				...
				if(cm.haveItem(4001028,selection*1)){
					cm.gainItem(4001028,-selection*1);
					cm.getPlayer().getStat().setMaxHp((cm.getPlayer().getStat().getMaxHp() + selection*50));
					cm.刷新状态();
					cm.dispose();
                }else{
					cm.sendOk("材料不足");
