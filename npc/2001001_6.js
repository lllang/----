...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "#r#e----------------------------------------------#k#n\r\n";
            text += " - 本环所需道具：[ #v 4031546##r#c 4031546##k/1500 ][ #v 4031636##r#c 4031636##k/50 ]  \r\n"
			text += " - 本环奖励道具：全属性+500#v1102137# x 1 #v2531000# x 30 #v2049600# x 30 #v2610003# x 30 #v2613008# x 30\r\n\r\n";
			text += "#L1##k" + aaa + "#d完成任务\r\n\r\n"

            cm.sendSimple(text);
        } else if (status == 1) {
            if (selection == 1) {//活动传送
                if (cm.getPlayer().getBossLog("任务名",10) >= 20 ) {
					cm.sendOk("你已经完成过一次了，无法再次完成");
					cm.dispose();
				} else if (cm.haveItem(4031546, 1500) == false) {
                    cm.sendOk("道具所需数量不足。");
                    cm.dispose();
				} else if (cm.haveItem(4031636, 50) == false) {
                    cm.sendOk("道具所需数量不足。");
                    cm.dispose();
				} else {
					cm.getPlayer().setBossLog("任务名",1);
					cm.gainItem(4031546, -1500);
					cm.gainItem(4031636, -50);
					cm.gainItem(2531000, 30);
					cm.gainItem(2049600, 30);
					cm.gainItem(2610003, 30);
					cm.gainItem(2613008, 30);
					cm.gainItem(1102137, 500, 500, 500, 500, 0, 0, 500, 500, 0, 0, 0, 0, 0, 0);
					cm.sendOk("成功完成！");
					cm.喇叭(2, "彩虹岛[" + cm.getPlayer().getName() + "]在拍卖新红包兑换出全属性500的披风点装！");
					cm.dispose();
				}
            }
        }
