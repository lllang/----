...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "#r#e----------------------------------------------#k#n\r\n";
            text += " - 所需道具：[ #v 3994036##r#c 3994036##k/1 ] [ #v 3994026##r#c 3994026##k/1 ] [ #v 3994183##r#c 3994183##k/1 ] \r\n"
			text += " - 奖励道具：抽奖#v4442100# x 1\r\n\r\n";
			text += "#L1##k" + aaa + "#d确认兑换\r\n\r\n"

            cm.sendSimple(text);
        } else if (status == 1) {
            if (selection == 1) {//活动传送
                if (cm.getPlayer().getBossLog("任务名",10) >= 1000 ) {
					cm.sendOk("你已经完成过一次了，无法再次完成");
					cm.dispose();
				} else if (cm.haveItem(3994036, 1) == false) {
                    cm.sendOk("道具所需数量不足。");
                    cm.dispose();
				} else if (cm.haveItem(3994026, 1) == false) {
                    cm.sendOk("道具所需数量不足。");
                    cm.dispose();
				} else if (cm.haveItem(3994183, 1) == false) {
                    cm.sendOk("道具所需数量不足。");
                    cm.dispose();
				} else {
					cm.getPlayer().setBossLog("任务名",1);
					cm.gainItem(3994183, -1);
