    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.getPS() == 8){// cm.getPS()  的意思是 读取跑商值如果等于1 就得出他跑商已经完成了第一环 就运行他进行第二环跑商!
                ...
                txt += "#L1##b收集老鼠陷阱#v4000095#X50个、向日葵子#v4000168#X50个交给我！！#l";
                cm.sendSimple(txt);
            }else{
...
cm.喇叭(2, "[" + cm.getPlayer().getName() + "]成功完成跑商任务第9轮，获得点卷200点奖励！");
                cm.sendOk("跑商第9环完成!恭喜获得金币=200000、经验=150000、点卷=200点\r\n\r\n然后你去找..神木村-仓库管理员-寇斯裤.进行下一环！");
                cm.dispose();
            }else{
                cm.sendOk("收集50个老鼠陷阱#v4000095#、50个向日葵子#v4000168#交给我!");
                cm.dispose();
            }
        }
    }
