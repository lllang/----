...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			textz += "#d#L5#升级光明天使之翼：#v1102723#（全属性3000双攻3000）#l\r\n\r\n#v4001024#*38   #v4251202#*99  #v4250002#*300  #v4000464#*32  #v4002001#*300\r\n";
			...
			if (selection == 0) {
				...
				if (cm.haveItem(4002001, 300) == false) {
					cm.sendOk("所需道具数量不足");
					cm.dispose();
					return;
				}
				if (cm.haveItem(4000464, 32) == false) {
					...
					cm.gainItem(1102723, 3000, 3000, 3000, 3000, 0, 0, 3000, 3000, 0, 0, 0, 0, 0, 0);
					cm.sendOk("锻造成功。");
					cm.dispose();
				}
			} else if (selection == 5) {
				...
				if (cm.haveItem(4000464, 32) == false) {
					...
					cm.gainItem(4000464, -32);
					cm.gainItem(4002001, -300);
					cm.gainItem(1102724, 3000, 3000, 3000, 3000, 0, 0, 3000, 3000, 0, 0, 0, 0, 0, 0);
					cm.sendOk("锻造成功。");
					cm.dispose();
				}
			}
