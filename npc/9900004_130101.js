...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            text += "#v1003861#   #e#r欢迎来到" + cm.ms() + "内测中心  #v1003861##k#n\r\n"
			text += "#r#e----------------------------------------------#k#n\r\n";
			text += "当前金币拥有：#r[" + cm.getPlayer().getMeso() + "]#k    目前点券拥有：#r["+cm.getPlayer().getCSPoints(1)+"]#k#n\r\n"
			text += "#r#e----------------------------------------------#k#n\r\n";
            //text += " - 所需道具：[ #v 4000031##r#c 4000031##k/1 ] \r\n"
			text += " - 内测奖励：#v2041213# x 3000 #v2614001# x 100  #v4002001# x 100  点卷100万\r\n\r\n";
			text += "#L1##k" + aaa + "#d完成任务\r\n\r\n"

            cm.sendSimple(text);
        } else if (status == 1) {
            if (selection == 1) {//活动传送
                if (cm.getPlayer().getBossLog("任务名",1) >= 1 ) {
					...
					//cm.gainNX(1000000);
					cm.喇叭(2, "彩虹岛[" + cm.getPlayer().getName() + "]在拍卖领取了内测大礼包！");
					cm.sendOk("成功完成！");
					cm.dispose();
				}
            }
