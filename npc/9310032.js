...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            for (i = 0; i < 10; i++) {
                text += "我这里是枫叶中介！\r\n\r\n";
                text += "" + 红色箭头 + "#L1##b#e使用#v4032733# （一个）兑换：枫叶#v4001126#=100个#l \r\n\r\n"//3
				text += "" + 红色箭头 + "#L5##b#e使用#v4032733# （一百）兑换：枫叶#v4001126#=10000个#l \r\n\r\n"//3
				text += "" + 红色箭头 + "#L2##b#e使用#v4310080# （一个）兑换：枫叶#v4001126#=8888个#l \r\n\r\n"//3
				text += "" + 红色箭头 + "#L4##b#e使用#v4001126# （9000个）兑换：枫叶#v4310080#=1个#l \r\n\r\n"//3
				text += "" + 红色箭头 + "#L3##b#e使用#v4310080# （三个）兑换：枫叶#v4001126#=23500个#l \r\n\r\n"//3
			cm.sendSimple(text);
            }
...
cm.喇叭(2, "[" + cm.getPlayer().getName() + "]在枫叶中介兑换[红色枫叶100个]，感谢光临~");
            cm.dispose();
			}else{
            cm.sendOk("枫叶不足无法换购！");
            cm.dispose();
...
cm.喇叭(2, "[" + cm.getPlayer().getName() + "]在枫叶中介兑换[红色枫叶10000个]，感谢光临~");
            cm.dispose();
			}else{
            cm.sendOk("枫叶不足无法换购！");
            cm.dispose();
...
cm.喇叭(1, "[" + cm.getPlayer().getName() + "]在枫叶中介兑换[红色枫叶8888个]，感谢光临~~");
            cm.dispose();
			}else{
            cm.sendOk("枫叶不足无法换购！");
            cm.dispose();
...
cm.喇叭(1, "[" + cm.getPlayer().getName() + "]在枫叶中介兑换[红色枫叶16000个]，感谢光临~~");
            cm.dispose();
			}else{
            cm.sendOk("枫叶不足无法换购！");
            cm.dispose();
