...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            text += "#v1003861#   #e#r欢迎来到" + cm.ms() + "内测礼包  #v1003861##k#n\r\n"
			text += "#r#e----------------------------------------------#k#n\r\n";
			//text += "当前金币拥有：#r[" + cm.getPlayer().getMeso() + "]#k    目前点券拥有：#r["+cm.getPlayer().getCSPoints(1)+"]#k#n\r\n"
			text += "#r#e----------------------------------------------#k#n\r\n";
            //text += " - 本环所需道具：[ #v 4000031##r#c 4000031##k/99 ] \r\n"
			text += " - 内测奖励道具：#v4001126# x 10000 #v 2531000# x 2 #v 2450000# x 1 #v 4170000# x 10\r\n\r\n";
			text += "#L1##k" + aaa + "#d领取奖励\r\n\r\n"

            cm.sendSimple(text);
        } else if (status == 1) {
            if (selection == 1) {//活动传送
                if (cm.getPlayer().getBossLog("任务名",10) >= 1 ) {
					...
					cm.喇叭(2, "彩虹岛[" + cm.getPlayer().getName() + "]领取公测礼包！");
					cm.sendOk("成功完成！");
					cm.dispose();
				}
            }
