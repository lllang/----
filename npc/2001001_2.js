...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "#r#e----------------------------------------------#k#n\r\n";
            text += " - 本环所需道具：[ #v 4000108##r#c 4000108##k/99 ] \r\n"
			text += " - 本环奖励道具：四维+20双攻+20#v1062007# x 1\r\n\r\n";
			text += "#L1##k" + aaa + "#d完成任务\r\n\r\n"
            ...
            if (selection == 1) {//活动传送
                if (cm.getPlayer().getBossLog("任务名",10) >= 10 ) {
					...
					cm.dispose();
				} else if (cm.haveItem(4000108, 99) == false) {
                    cm.sendOk("道具所需数量不足。");
                    cm.dispose();
				} else {
					cm.getPlayer().setBossLog("任务名",1);
					cm.gainItem(4000108, -99);
					cm.gainItem(1062007, 20, 20, 20, 20, 0, 0, 20, 20, 0, 0, 0, 0, 0, 0);
					cm.sendOk("成功完成！");
