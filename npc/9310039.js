...
function action(mode, type, selection) {
    if (mode == -1) {
		...
		if (cm.getPlayer().getClient().getChannel() != 1 && cm.getPlayer().getClient().getChannel() != 2) {
			cm.sendOk("妖僧只有在频道 #r1#k 和 #r2#k可以挑战");
			cm.dispose();
			return;
	    }
        if (mode == 1)
        ...
        if (status == 0) {
            ...
            if (selection == 0) {
                ...
                if ( cm.getQuestStatus(8534) != 0 ) {
                    ...
                    while (it.hasNext()) {
                        var cPlayer = it.next();
                        if ((cPlayer.getLevel() >= 80 && cPlayer.getLevel() <=255 ) || cPlayer.getJobId() == 000) {
                            levelValid += 1;
                        } else {
                            next = false;
                        }
                    ...
                    if (next) {
                        ...
                        cm.sendOk("等级尚未达到 #r80#k 或者已经超过 #r255#k");
                    }
                }
                cm.dispose();
            }
