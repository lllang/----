...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			var textz = "#r此处可以升级装备110次#k：\r\n";

			textz += "#L0#消耗 #b10000点券#k 为装备提升装备升级次数。\r\n\r\n";

			textz += "#L3#消耗 #b100000点券#k  为装备提升装备升级10次数。\r\n\r\n";

			//textz += "#L4#消耗 #b#v4442001##k*1 兑换 #v4001126# X 500\r\n\r\n";
			...
			if (sj == 1) {
				if (cm.getPlayer().getCSPoints(1) < 10000) {
					...
					cm.gainNX(-10000);
					var selStr = "强化成功";
					cm.喇叭(1," 玩家:<"+cm.getName()+">成功消耗10000点卷使装备增加1次强化次数~!");
					cm.sendOk(selStr);
					cm.dispose();
				}
				...
				if (cm.haveItem(2049124, 强化道具数量) == false) {
					...
					if (随机值 < 强化几率) {
						...
						if (cm.haveItem(4001126, 1001) == false) {
							cm.sendOk("道具不足。");
							cm.dispose();
						} else if ((装备最大升级次数 - 可升级次数 - 已升级次数) < 1) {
							cm.sendOk("当前装备无法升级。");
						...
						if (cm.getPlayer().getCSPoints(1) < 100000) {
							...
							cm.gainNX(-100000);
							var selStr = "强化成功";
							cm.喇叭(1," 玩家:<"+cm.getName()+">成功消耗100000点卷使装备增加10次强化次数~!");
							cm.sendOk(selStr);
							cm.dispose();
						}
