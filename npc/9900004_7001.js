...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "#L50##e#d" + 蓝色箭头 + "蓝蘑菇王副本#l#L51##d " + 蓝色箭头 + "千年树精副本#l\r\n\r\n"//3
		 ...
		 } else if (selection == 50) {//木妖王
            if (cm.getLevel() < 20 ) {  
            cm.sendOk("本地图限制等级20级。您的能力没有资格挑战蓝蘑菇王副本");
                cm.dispose();
              }else{
			cm.warp(800010100);  
		...
		} else if (selection == 51) {//木妖王
            if (cm.getLevel() < 20 ) {  
            cm.sendOk("本地图限制等级20级。您的能力没有资格挑战千年树精副本");
                cm.dispose();
              }else{
			cm.warp(541020700);  
						cm.喇叭(1, "[BOSS传送]：玩家" + cm.getPlayer().getName() + "开始挑战千年树精副本，大家都来战个痛快吧~");
				cm.dispose();
                return;
	      }
		} else if (selection == 55) {//木妖王
            if (cm.getLevel() < 100 ) {  
            cm.sendOk("本地图限制等级100级。您的能力没有资格挑战千年树精副本");
                cm.dispose();
              }else{
			cm.warp(702070400);  
						cm.喇叭(1, "[BOSS传送]：玩家" + cm.getPlayer().getName() + "开始挑战少林妖僧副本，大家都来战个痛快吧~");
				cm.dispose();
                return;
	      }
		} else if (selection == 52) {//木妖王
            if (cm.getLevel() < 180 ) {  
            cm.sendOk("本地图限制等级180级。您的能力没有资格挑战PB王副本");
                cm.dispose();
              }else{
			cm.warp(270050000);  
						cm.喇叭(1, "[BOSS传送]：玩家" + cm.getPlayer().getName() + "开始挑战PB副本，大家都来战个痛快吧~");
				cm.dispose();
                return;
	      }
		} else if (selection == 53) {//木妖王
            if (cm.getLevel() < 150 ) {  
            cm.sendOk("本地图限制等级150级。您的能力没有资格挑战PB王副本");
                cm.dispose();
              }else{
			cm.warp(803001200);  
						cm.喇叭(1, "[BOSS传送]：玩家" + cm.getPlayer().getName() + "开始挑战绯红副本，大家都来战个痛快吧~");
				cm.dispose();
                return;
	      }
		} else if (selection == 54) {//木妖王
            if (cm.getLevel() < 70 ) {  
            cm.sendOk("本地图限制等级70级。您的能力没有资格挑战大海兽副本");
                cm.dispose();
              }else{
			cm.warp(240040401);  
						cm.喇叭(1, "[BOSS传送]：玩家" + cm.getPlayer().getName() + "开始挑战大海兽副本，大家都来战个痛快吧~");
				cm.dispose();
                return;
	      }
